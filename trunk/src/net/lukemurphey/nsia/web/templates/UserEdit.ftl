    ...
    <table class="DataTable">
        <form onsubmit="showHourglass()" method="post">
            <tr class="<#if (form_errors?? && form_errors.fieldHasError("Username"))>ValidationFailed<#else>Background1</#if>">
                ...
                <td><input class="textInput" size="32" type="text" name="Username" value="<#if request.getParameter("Username")??>${request.getParameter("Username")}<#elseif user??>${user.userName}</#if>"></td>
            </tr>
            <tr class="<#if (form_errors?? && form_errors.fieldHasError("Fullname"))>ValidationFailed<#else>Background1</#if>">
                ...
                <td><input class="textInput" size="32" type="text" name="Fullname" value="<#if request.getParameter("Fullname")??>${request.getParameter("Fullname")}<#elseif user?? && user.fullname??>${user.fullname}</#if>"></td>
            </tr>
            <tr class="<#if (form_errors?? && form_errors.fieldHasError("EmailAddress"))>ValidationFailed<#else>Background1</#if>">
                ...
                <td><input class="textInput" size="32" type="text" name="EmailAddress" value="<#if request.getParameter("EmailAddress")??>${request.getParameter("EmailAddress")}<#elseif user?? && user.emailAddress??>${user.emailAddress}</#if>"></td>
            </tr>
            <#if !user??>
