...
import net.lukeMurphey.nsia.scanRules.DefinitionSet.DefinitionType;
...
public class DefinitionArchive {
	 ...
	 */
	public Definition getDefinition(int localId) throws NotFoundException{
	 ...
	 * @throws DisallowedOperationException 
	 */
	public void removeByID( int localId ) throws SQLException, NoDatabaseConnectionException, DisallowedOperationException{
		// 0 -- Precondition check
		if( application == null){
		...
		synchronized (archive) {
			...
			for(int c = 0; c < definitionSet.size() && indexOfDefinition == -1; c++){
				definition = definitionSet.get(c);
				if( definition.getLocalID() == localId ){
					indexOfDefinition = c;
				}
			}
