...
<#assign content>
    ...
    <form action="<#if !group??><@url name="group_editor" args=["New"]/><#else><@url name="group_editor" args=["Edit", group.groupId]/></#if>" method="post">
        <#if group??><input type="hidden" name="GroupID" value="${group.groupId}"></#if>
        <table class="DataTable">
            <tr class="<#if (form_errors?? && form_errors.fieldHasError("Name"))>ValidationFailed<#else>Background1</#if>">
                ...
                <td><input class="textInput" style="width: 350px;" type="text" name="Name" value="<#if request.getParameter("Name")??>${request.getParameter("Name")}<#elseif group??>${group.groupName}</#if>"></td>
            </tr>
            <tr style="vertical-align:top" class="<#if (form_errors?? && form_errors.fieldHasError("Description") )>ValidationFailed<#else>Background1</#if>">
                ...
                <td><textarea style="width: 350px;" rows="8" name="Description"><#if request.getParameter("Description")??>${request.getParameter("Description")}<#elseif group??>${group.description}</#if></textarea></td>
            </tr>
