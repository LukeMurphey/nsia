...
public class HttpDefinitionScanResult extends ScanResult {
	...
	/*
	private HttpSignatureScanResult( ResultSet result, Application application ) throws ScanResultLoadFailureException{
		super(result);
		loadFromScanResult(result, application);
	}
	*/
	 ...
	 */
	public int archive(){
		//SpecimenSnapshot snapshot = new SpecimenSnapshot( this. );
		return -1;
	}
	...
	public static MaxMinCount getScanResultInfo( long parentScanResultID, SignatureScanResultFilter filter, Application application ) throws SQLException, NoDatabaseConnectionException{
		...
		try{
			...
			else{//if( filter.getContentType() == null && filter.getSignatureName() == null ){
	...
	public long saveLinkedResultToDatabase(Connection connection, long scanResultId) throws SQLException {
		...
		try{
			...
			// 3 -- Save the data for the signature matches
